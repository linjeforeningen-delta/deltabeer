...
import dev.stonegarden.deltahouse.user.UserPropertyDAO
...
internal class WalletServiceTest {
    ...
    inner class PurchaseFunctionTest {
        ...
        fun `user not found during purchase`() {
            every { mockUserService.getUserDAOByCardId(any()) } throws dev.stonegarden.deltahouse.exceptions.UserNotFoundException()
            assertThrows<dev.stonegarden.deltahouse.exceptions.UserNotFoundException> { walletService.purchase(1, 10) }
        }

        @Test
        fun `exceeding purchase value`() {
            val exception = assertThrows<dev.stonegarden.deltahouse.exceptions.InvalidTransactionException> {
            ...
            listOf(
                ...
                dynamicTest(
                    ...
                    if (errorMessage == null) {
                        ...
                        val exception =
                            assertThrows<dev.stonegarden.deltahouse.exceptions.InvalidTransactionException> {
                                walletService.purchase(
                                    1,
                                    value.toShort()
                                )
    ...
    inner class DepositFunctionTests {
        ...
        fun `user not found during deposit`() {
            every { mockUserService.getUserDAOByCardId(any()) } throws dev.stonegarden.deltahouse.exceptions.UserNotFoundException()
            assertThrows<dev.stonegarden.deltahouse.exceptions.UserNotFoundException> { walletService.deposit(1, 10) }
        }
        ...
        fun `cash balance integer overflow prevention`() {
            ...
            var exception = assertThrows<dev.stonegarden.deltahouse.exceptions.InvalidTransactionException> {
                walletService.deposit(
                    3,
                    1
                )
