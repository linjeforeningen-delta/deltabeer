...
internal class UserIntegrationTests {
    ...
    inner class UserIntegrationTests {
        ...
        fun `create user test`() {
            ...
            assertThrows<dev.stonegarden.deltahouse.exceptions.CardIsAlreadyRegisteredException>("We should not be able to register a new user with an already existing cardId.") {
                userController.createUser(userDTO.copy(cardId = usedCardIds.random()))
            }

            val uniqueCardId = newCardId()
            assertThrows<dev.stonegarden.deltahouse.exceptions.UserNotFoundException>("We should not be able to find the user before it has been created.") {
            ...
            assertThrows<dev.stonegarden.deltahouse.exceptions.EmailIsAlreadyRegisteredException>("Supplied e-mail must be unique.") {
        ...
        fun `update user details test`() {
            ...
            assertThrows<dev.stonegarden.deltahouse.exceptions.EmailIsAlreadyRegisteredException>("New e-mail must not already be registered.") {
        ...
        fun `change user card ID test`() {
            ...
            assertThrows<dev.stonegarden.deltahouse.exceptions.CardIsAlreadyRegisteredException>("Card ID must be unique.") {
            ...
            assertThrows<dev.stonegarden.deltahouse.exceptions.UserNotFoundException>("User is not found when searching for an unknown email address.") {
                userController.changeUserCardId(
                    unknownEmailAddress,
                    newCardId
                )
        ...
        fun `delete user test`() {
            assertThrows<dev.stonegarden.deltahouse.exceptions.UserNotFoundException>("Can't delete a non-existing user.") {
            ...
            assertThrows<dev.stonegarden.deltahouse.exceptions.UserIsDeletedException>("Should not be able to find deleted user.") {
                userController.getUserByCardId(
                    userToDelete.cardId
                )
            }
