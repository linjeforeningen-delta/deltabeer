...
internal class UserServiceTest {
    ...
    fun getUserByCardId() {
        ...
        assertThrows<dev.stonegarden.deltahouse.exceptions.UserNotFoundException>("We should get an exception if no user has the given card ID.") {
            userService.getUserByCardId(
                notRegisteredCardId
            )
        }

        assertThrows<dev.stonegarden.deltahouse.exceptions.UserIsDeletedException>("We should get an exception if user has been soft deleted.") {
            userService.getUserByCardId(
                testUserC.cardId
            )
        }
    ...
    fun createUser() {
        ...
        assertThrows<dev.stonegarden.deltahouse.exceptions.CardIsAlreadyRegisteredException>("We should not be able to register the same card ID twice") {
            userService.createUser(
                newUser,
                "Should fail"
            )
        }
        assertThrows<dev.stonegarden.deltahouse.exceptions.EmailIsAlreadyRegisteredException>("We should not be able to register the same email twice.") {
            userService.createUser(
                newUser,
                "Should fail"
            )
    ...
    fun updateUserDetails() {
        ...
        assertThrows<dev.stonegarden.deltahouse.exceptions.EmailIsAlreadyRegisteredException>("We shouldn't be able to change an email to one already registered") {
    ...
    fun changeUserCardId() {
        ...
        assertThrows<dev.stonegarden.deltahouse.exceptions.CardIsAlreadyRegisteredException>("We shouldn't be able to update the card ID to a card ID already registered.") {
        ...
        assertThrows<dev.stonegarden.deltahouse.exceptions.UserNotFoundException>("We can't update the card ID if we can't identify the user by email.") {
            userService.changeUserCardId(
                testUserA.email,
                newCardId,
                "Test"
